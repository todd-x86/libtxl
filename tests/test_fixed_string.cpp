...
TXL_UNIT_TEST(string_block)
{
    txl::string_block<16> account{};
    assert(account == "");
    ...
    txl::string_block<8> data{"12345678"};
    assert(data == "12345678");

    auto shakezula = std::string{"Master Shake"};
    txl::string_block<20> athf{shakezula};
    assert(athf == "Master Shake");

    using cartoon_name = txl::string_block<14>;
    std::array<cartoon_name, 4> aqua_teens {
        "Shake",
        "Frylock",
        "Meatwad",
